class User < ApplicationRecord
  ...
  scope :with_two_names, ->(first, second){ where('(LOWER(first_name) LIKE :other_name AND LOWER(last_name) LIKE :name ) OR (LOWER(first_name) LIKE :name AND LOWER(last_name) LIKE :other_name)' , name: "%#{first.downcase}%", other_name: "%#{second.downcase}%") }
